...
Instructions for Invoice Analysis and Voucher Type Selection:

1. Identify the Voucher Type based on the company perspective:
    - If the invoice has stock items, create a Sales or Purchase entry from the perspective of "{company_name}".
    - If the invoice does not involve a sale or purchase of stock items or you are unsure, use Journal as the Voucher Type.

2. Output Format for Different Voucher Types:
    - Sales/Purchase Entry (for invoices with stock items):
        ...
        - Line 2: Actual data for the invoice, in the same order as the headers in Line 1.
    ...
    - Journal Entry (for other types of transactions):
        - Line 1: Column headers for journal entry data in this order:
        "Voucher Type","Voucher Date","Narration"
        - Line 2: Actual data for the journal entry, in the same order as the headers in Line 1.
        - Line 3: Column headers for journal entry account details in this order:
        "Account Name","Account Address","Account State","Account GSTIN","Account Group","Debit Amount","Credit Amount"
        - Line 4 and onwards: Actual data for each account, in the same order as the headers in Line 3.
3. Output no other information except for the csv data.

Important Rules:
- In invoice text, each page data is separated by 3 new lines.
- Tax Handling: For invoices with CGST/SGST, add them together to find the total tax rate.
...
- Journal Entry Rules:
    ...
    - Ensure total Debit and Credit amounts are equal.
